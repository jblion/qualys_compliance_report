...
def split_file(input_filename, output_folder):
    try:
        with open(input_filename, "r", encoding="utf-8") as file:
            sections = {"summary": [], "assets": [], "results": [], "errors": []}
        ...
        final_folder = Path(input_filename).stem
        os.makedirs(os.path.join(output_folder,final_folder), exist_ok=True)
        for section, lines in sections.items():
            if lines:  # Vérifie qu'il y a bien du contenu avant d'écrire
                output_filename = f"{section.replace(' ', '_')}.csv"
                with open(os.path.join(output_folder,final_folder,output_filename), "w", encoding="utf-8") as out_file:
